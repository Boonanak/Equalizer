...
%
% IF R1 = R2 and  R3 = R4
%   If gama > 0.5, the gain is always less than 1, so it is a band-stop filter
%   If gama < 0.5, the gain is always greater than 1, so it is a band-pass filter
%   If gama = 0.5, the gain is always equal to 1
%
% Notice: Input references should all be considered as numbers


function [fc, Gc, fc1, fc2] = Equalizer_filter_gain(R1, R2, R3, R4, R5, C1, ...
    ...
    if(strcmp(command, 'SingleCalculation'))
        if ((gama > 0.5 && max_gain >= (1 / sqrt(2))) || (gama < 0.5 && max_gain <= sqrt(2)))
